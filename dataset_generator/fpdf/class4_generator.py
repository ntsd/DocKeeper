        ...
        with open(infile) as file:
            for linea in file.readlines():
                ...
                for i,v in enumerate(linea.split(";")):
                    ...
                    kargs[keys[i]] = v
                x=kargs['x1']
                kargs['x1'] = 200-kargs['x2']
                kargs['x2'] = 200-x
                kargs['y1'] = 290-kargs['y1']
                kargs['y2'] = 290-kargs['y2']
...
if __name__ == "__main__":
    f = Form("invoice2.csv")
    f.set("logo","logo.png")
    # f.render("class1/invoice.pdf")
    for fileNo in range(300):
        f.set("company_header1", random_string(random.randint(4,12)))
        f.set("company_header2", random_string(random.randint(4,12)))
        f.set("company_footer1", random_string(random.randint(4,12)))
        f.set("company_footer2", random_string(random.randint(4,12)))
        for i in range(1,random.randint(2,24)):
            f.set("item_description{:02d}".format(i), random_string(random.randint(10,20)))
            f.set("item_code{:02d}".format(i), random_string(5))
            f.set("item_amount{:02d}".format(i), str(random.randint(1,50)))
            f.set("item_price{:02d}".format(i), str(random.randint(1,100)))
            f.set("item_quantity{:02d}".format(i), str(random.randint(1,10)))
        f.set("net", str(random.randint(100,200)))
        f.set("total", str(random.randint(100,200)))

        f.render("class4/invoice{}.pdf".format(fileNo))
